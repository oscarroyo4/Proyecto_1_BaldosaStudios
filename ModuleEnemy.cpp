...
update_status ModuleEnemy::Update()
{
	...
	case ENEMY_JUMP:
		if (jumpEnable == true) {
			jumpEnable = false;
			jump.Reset();
			if (Mix_PlayChannel(-1, jumpfx1, 0) == -1)
			{
	...
	case ENEMY_KICK:
		if (kickEnable == true) {
			...
			kick.Reset();
			kick_timer = 1;
			if (Mix_PlayChannel(-1, kickfx1, 0) == -1)
			{
				LOG("Could not play select sound. Mix_PlayChannel: %s", Mix_GetError());
			}
	...
	case ENEMY_PUNCH:
		if (punchEnable == true) {
			...
			punch.Reset();
			punch_timer = 1;
			if (Mix_PlayChannel(-1, punchfx1, 0) == -1)
			{
				LOG("Could not play select sound. Mix_PlayChannel: %s", Mix_GetError());
			}
	...
	if (defeat_timer > 0)
	{
		...
		if (win_timer == 4)
		{
			if (Mix_PlayChannel(-1, defeatfx1, 0) == -1)
			{
				LOG("Could not play select sound. Mix_PlayChannel: %s", Mix_GetError());
			}
		}
	...
	if (win_timer > 0)
	{
		...
		if (win_timer == 4)
		{
			if (Mix_PlayChannel(-1, winfx1, 0) == -1)
			{
				LOG("Could not play select sound. Mix_PlayChannel: %s", Mix_GetError());
			}
		}
